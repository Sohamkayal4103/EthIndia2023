...
contract UserSide {
    ...
    address EPNS_COMM_ADDRESS= 0x0C34d54a09CFe75BCcd878A469206Ae77E0fe6e7; //address for sepolia
    ...
    function createUser(string memory _userName,string memory _userEmail,string memory _description,string memory _profileImage,address _userWalletAddress) public {
        totalUsers++;
    ...
    function updateUser(uint256 _userId,string memory _userName,string memory _userEmail,string memory _description,string memory _profileImage,address _userWallet) public {
        user memory u1 = user(_userId,_userName,_userEmail,_description,_profileImage,_userWallet);
        userIdtoUser[_userId] = u1;
        userWallettoUser[_userWallet] = totalUsers;
    }

    function createDao(string memory _daoName,string memory _daoDescription,uint256 _joiningThreshold,address _joiningTokenAddress,bool _isPrivate,address _userWalletAddress) public {
        totalDaos++;
        uint256 creatorId = userWallettoUser[_userWalletAddress];
        require(creatorId != 0,"User is not registered into the system");
        dao memory d1 = dao(totalDaos,creatorId,_daoName,_daoDescription,_joiningThreshold,_joiningTokenAddress,_isPrivate);
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "3", // define notification type:  https://push.org/docs/notifications/build/types-of-notification (1, 3 or 4) = (Broadcast, targeted or subset)
                        "+", // segregator
                        "Congratulations!", // this is notification title
                        "+", // segregator
                        "Your dao has been created" // notification body
                    )
                )
            )
        );
    ...
    function updateDao(uint256 _daoId,string memory _daoName,string memory _daoDescription,uint256 _joiningThreshold,address _joiningTokenAddress,bool _isPrivate,address _userWalletAddress) public {
        uint256 tempCreatorId = userWallettoUser[_userWalletAddress];
        require(tempCreatorId == daoIdtoDao[_daoId].creator,"Only owner of the dao can change the dao Information");
        dao memory d1 = dao(_daoId,tempCreatorId,_daoName,_daoDescription,_joiningThreshold,_joiningTokenAddress,_isPrivate);
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "Congratulations!", // this is notification title
                        "+", // segregator
                        "Dao Information Updated" // notification body
                    )
                )
            )
        );
    ...
    function createProposal(string memory _proposalTitle,string memory _proposalDescription,uint256 _votingThreshold,uint256 _daoId,address _governanceTokenAddress,address _userWalletAddress) public {
        totalProposals++;
        ...
        proposal memory p1 = proposal(totalProposals,_proposalTitle,_proposalDescription,tempProposerId,_votingThreshold,0,_daoId,_governanceTokenAddress);
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "Congratulations!", // this is notification title
                        "+", // segregator
                        "A proposal has been created" // notification body
                    )
                )
            )
        );
    ...
    function promoteUser(uint256 _userId,uint256 _daoId,address _userWalletAddress) public {
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "Congratulations!", // this is notification title
                        "+", // segregator
                        "You have been promoted" // notification body
                    )
                )
            )
        );
    ...
    function openVoting(uint256 _proposalId,address _callerWalletAddress) public {
    ...
    function closeVoting(uint256 _proposalId,address _callerWalletAddress) public {
    ...
    function voteForProposal(uint256 _proposalId,bool _voteFor,address _callerWalletAddress) public {
        address funcCaller = _callerWalletAddress;
        uint256 tempDaoId = proposalIdtoproposal[_proposalId].daoId;
        require(checkMembership(tempDaoId,_callerWalletAddress),"Only members of the can vote");
        uint256 tempProposalStage = proposalIdtoproposal[_proposalId].proposalStage;
        require(tempProposalStage == 1,"Voting is either not started or its already over");
        //uint256 tempVotingthreshold = proposalIdtoproposal[_proposalId].votingThreshold;
        address votingTokenAddress = proposalIdtoproposal[_proposalId].governanceTokenAddress;
    ...
    function getDaoMemberId(uint256 _daoId,uint256 _index) public view returns(uint256) {
        return daoIdtoMembers[_daoId][_index];
    }

    function getDaoProposalId(uint256 _daoId,uint256 _index) public view returns (uint256) {
        return daoIdtoProposals[_daoId][_index];
    }

    function getTotalSupportVotes(uint256 _proposalId) public view returns (uint256) {
        return proposalIdtoSupporters[_proposalId].length;
    }    

    function getTotalOppositionVotes(uint256 _proposalId) public view returns (uint256) {
        return proposalIdtoOpposers[_proposalId].length;
    }

    function getTotalVotes(uint256 _proposalId) public view returns (uint256) {
        return getTotalSupportVotes(_proposalId) + getTotalOppositionVotes(_proposalId);
    }

    function getSupporterId(uint256 _proposalId,uint256 _index) public view returns(uint256) {
        return proposalIdtoSupporters[_proposalId][_index];
    }

    function getOpposerId(uint256 _proposalId,uint256 _index) public view returns(uint256) {
